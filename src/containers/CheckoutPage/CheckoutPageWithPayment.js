...
// ---- CASH process constants (⚠️ adapte l’alias si besoin)
 ...
 * Used by both speculate transition and real transition.
 */
...
/** Keep this export in case parent still imports it */
...
/** Simple billing form used for CASH flow */
const BillingForm = ({ submitting, currentUser, onSubmit }) => {
  ...
  return (
    <form className={css.billingForm} onSubmit={submit}>
      ...
      <H4 className={css.subHeading} style={{ marginTop: 16 }}>
        <FormattedMessage defaultMessage="Informations additionnelles" />
      </H4>
      ...
      <PrimaryButton className={css.submitButton} type="submit" inProgress={submitting}>
        <FormattedMessage defaultMessage="Demander en espèces" />
      </PrimaryButton>
    </form>
  );
};
...
export const CheckoutPageWithPayment = props => {
  ...
  const {
    ...
    title,
    config,
    // <- ajouté: 'stripe' | 'cash' selon la route
  ...
  // 🔎 Currency guard: for CASH we do not require Stripe compatibility
  ...
  const handleSubmitCash = async formValues => {
    ...
    try {
      ...
      // Keep useful info in protectedData
      ...
      const res = await props.onInitiateOrder(
        ...
        true // transition likely privileged in your process.edn
      );

      const created = res?.payload?.data || res?.data || res;

      const orderDetailsPath = pathByRouteName('OrderDetailsPage', props.routeConfiguration, {
        id: created.id.uuid,
      });
      ...
      <div className={css.contentContainer}>
        ...
        <div className={css.orderFormContainer}>
          ...
          <section className={css.paymentContainer}>
            ...
            {/* Stripe block only when card flow */}
            {showPaymentBlock && !isCash ? (
              <StripePaymentForm
            ...
            {/* Billing + note form always visible in CASH flow */}
            {showPaymentBlock && isCash ? (
              <BillingForm
...
CheckoutPageWithPayment.propTypes = {
  ...
  paymentMethod: propTypes.string, // 'stripe' | 'cash'
};
